...
function sendPasswordResetEmail($toEmail, $toName, $code) {
    $apiKey = defined('RESEND_API_KEY') ? RESEND_API_KEY : '';
    ...
    $htmlContent = getPasswordResetEmailTemplate($toName, $code);

    $data = json_encode([
        'from' => 'SelarasKas <onboarding@resend.dev>',
        'to' => [$toEmail],
        'subject' => "🔑 Reset Password SelarasKas: $code",
        'html' => $htmlContent,
    ]);
    ...
    if ($response === false) {
        error_log('Resend API request failed for password reset');
        return false;
    }
...
function getPasswordResetEmailTemplate($name, $code) {
    ...
    $codeBoxes = '';
    foreach ($digits as $d) {
        $codeBoxes .= '<td style="width:42px;height:52px;text-align:center;font-size:28px;font-weight:700;font-family:\'Inter\',monospace;background:#1a1f2e;color:#f59e0b;border-radius:10px;border:2px solid #2a2f3e;letter-spacing:0">' . $d . '</td>';
    }
...
<table width="480" cellpadding="0" cellspacing="0" style="background:#111627;border-radius:16px;border:1px solid #1e2338;overflow:hidden;max-width:90%">
    <!-- Header -->
    <tr><td style="background:linear-gradient(135deg,#f59e0b 0%,#d97706 50%,#b45309 100%);padding:32px 40px;text-align:center">
        <table cellpadding="0" cellspacing="0" style="margin:0 auto"><tr>
            <td style="background:rgba(255,255,255,0.2);border-radius:12px;padding:10px;vertical-align:middle">
                <img src="https://img.icons8.com/fluency/48/lock-2.png" width="28" height="28" alt="🔑" style="display:block">
            </td>
            <td style="padding-left:12px;vertical-align:middle">
                <span style="font-size:22px;font-weight:800;color:#fff;letter-spacing:-0.5px">SelarasKas</span>
            </td>
        </tr></table>
        <p style="color:rgba(255,255,255,0.85);margin:10px 0 0;font-size:14px">Reset Password</p>
    </td></tr>
    
    <!-- Body -->
    <tr><td style="padding:36px 40px">
        <h2 style="color:#f1f5f9;margin:0 0 8px;font-size:20px;font-weight:700">Halo, ' . htmlspecialchars($name) . '! 🔑</h2>
        <p style="color:#94a3b8;margin:0 0 28px;font-size:14px;line-height:1.6">Kamu meminta reset password untuk akun SelarasKas. Masukkan kode di bawah ini:</p>
        
        <!-- OTP Code -->
        <table cellpadding="0" cellspacing="6" style="margin:0 auto 28px"><tr>' . $codeBoxes . '</tr></table>
        
        <p style="color:#64748b;font-size:12px;text-align:center;margin:0 0 28px">⏱ Kode ini berlaku selama <strong style="color:#f59e0b">15 menit</strong></p>
        
        <div style="background:#1a1f2e;border-radius:10px;padding:16px 20px;border-left:3px solid #f59e0b">
            <p style="color:#94a3b8;margin:0;font-size:13px;line-height:1.5">
                ⚠️ Jika kamu tidak meminta reset password, abaikan email ini. Password kamu tetap aman.
            </p>
        </div>
    </td></tr>
